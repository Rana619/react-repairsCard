...
const signupSchema = yup.object().shape({
    ...
    accessToken: yup.string().required("Access token should be required")
});
...
export default function AdminAuth(props) {
    ...
    const onSignupSubmit = async (e) => {
        ...
        await signupSchema
            ...
            .then(async (data) => {
                ...
                await handleEmailPasswordSignup(dispatch, signupMail, signupPassword, displayName, true, accessToken);
            })
            .catch((err) => {
                console.log(err)
    ...
    const onLoginSubmit = async (e) => {
        ...
        await signinSchema
            .validate(formData, { abortEarly: false })
            .then(async (data) => {
                await handleEmailPasswordLogin(dispatch, loginMail, loginPassword, userNotFoundErr, true);
            })
            .catch((err) => {
                let errorArr = err?.inner;
                let obj = {};
            ...
            <div className={classes.mainCont} >
                <div className={classes.leftSide}>
                    <form className={classes.signupCont} onSubmit={onSignupSubmit} >
                        ...
                        />
                        {signupErrorObj?.accessToken ? (
                            <p className={classes.errorText} >{signupErrorObj?.accessToken}</p>
                        ) : (
                        ...
                        <Button className={classes.googleBtn}
                            onClick={(el) => { handleGoogleSignup(dispatch, true, accessToken) }}
                        ...
                        <Button className={classes.facebookBtn}
                            onClick={() => { handleFacebookSignup(dispatch, true, accessToken) }}
                            disabled={accessToken ? false : true}
                        >
                            <div className={classes.btnInnerCont} >
                        ...
                        <div className={classes.orCont} style={{ margin: "5px 0px 15px" }} >
                            <div></div>
                    ...
                    <Slide direction="right" in={isSignin} mountOnEnter unmountOnExit>
                        <form className={classes.signinCont} onSubmit={onLoginSubmit} >
                            ...
                            <Button className={classes.googleBtn}
                                onClick={(el) => { handleGoogleLogin(dispatch, true) }}
                            >
                                <div className={classes.btnInnerCont} >
                                    <FcGoogle style={{ fontSize: "20px" }} />
                            ...
                            <Button
                                ...
                                onClick={() => { handleFacebookLogin(dispatch, true) }}
                            >
                                <div className={classes.btnInnerCont} >
                                    <BsFacebook style={{ fontSize: "20px" }} />
